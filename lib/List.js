...
var List = function () {
  ...
  _createClass(List, [{
    ...
    value: function add(data, i) {
      if (!isNaN(i) && (0, _check.isNotUndefined)([data, i]) && (0, _check.isNotNull)([data, i])) {
        if (i === 0) {
    ...
    value: function toArray() {
      if (this.isEmpty()) {
        ...
        return [].concat(_toConsumableArray(this.array));
      }
    }
     ...
     * @return the index of the element or -1 if the element isn't in the list
     * @example
    ...
    key: 'indexOf',
    value: function indexOf(key, comparer) {
      if ((0, _check.isNotUndefined)(key) && (0, _check.isNotNull)(key) && (0, _check.isUndefined)(comparer)) {
        var idx = -1;
        for (var i = 0; i < this.array.length; i++) {
          if (this.array[i] === key) {
            idx = i;
        ...
        return idx;
      } else if ((0, _check.isNotUndefined)([key, comparer]) && (0, _check.isNotUndefined)([key, comparer]) && (0, _check.isFunction)(comparer)) {
        var _idx = -1;
        for (var _i = 0; _i < this.array.length; _i++) {
          if (comparer(key, this.array[_i])) {
            _idx = _i;
            break;
          }
        }
        return _idx;
      } else if ((0, _check.isNull)(key)) {
        throw _errors2.default["401"];
      } else if ((0, _check.isUndefined)(key)) {
        throw _errors2.default["402"];
      } else if ((0, _check.isNull)(comparer)) {
        throw _errors2.default["501"];
      } else if ((0, _check.isNotFunction)(comparer)) {
        throw _errors2.default["503"];
      } else {
        throw _errors2.default["001"];
      }
    }
    ...
    value: function sort(comparer) {
      if (this.isEmpty()) {
        throw _errors2.default["500"];
      } else {
        if ((0, _check.isNotUndefined)(comparer) && (0, _check.isNotNull)(comparer) && (0, _check.isFunction)(comparer)) {
          var vec = [].concat(_toConsumableArray(this.array));
          return this.builder(vec.sort(comparer));
        } else if ((0, _check.isNull)(comparer)) {
          throw _errors2.default["501"];
        } else if ((0, _check.isUndefined)(comparer)) {
          comparer = function comparer(x, y) {
            return x > y;
          };
          var _vec = [].concat(_toConsumableArray(this.array));
          return this.builder(_vec.sort(comparer));
        } else if ((0, _check.isNotFunction)(comparer)) {
          throw _errors2.default["503"];
        } else {
          throw _errors2.default["001"];
